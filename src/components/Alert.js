...
export default function Alert(props) {
  ...
  function simulation_start() {
    axios
      .get('https://765rrgmzf2.execute-api.ap-northeast-1.amazonaws.com/api')
      .then((res) => {
    ...
    axios
      .get(
        `https://765rrgmzf2.execute-api.ap-northeast-1.amazonaws.com/simulation`,
      )
      .then((res) => {
    ...
    return (
      <>
        <div onClick={() => props.isAlert(false)} className='overlay'>
          ...
          >
            <h1 className='text-2xl flex justify-center text-red-600 mb-12'>
              {props.message}
            </h1>
            <h2 className='text-xl flex justify-center mb-4'>{props.alert}</h2>
            <div className='flex justify-around'>
              {isClickShow ? (
                <p className='bg-gray-400 text-white px-16 py-1 rounded-3xl '>
                  15秒お待ちください
                </p>
              ) : (
                <>
                  <button
                    onClick={() => props.setIsAlert(false)}
